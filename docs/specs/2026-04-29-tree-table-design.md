...
- **Manual smoke item ("navigate the demo") shipped as a playground example only.** The spec recommended adding a TreeTable section to `examples/reacterm-demo.tsx`; the executing agent descoped that demo edit to keep the scope tight. The `playground/examples/tree-table.tsx` file covers the same surface (3 levels, 4 columns, expand/collapse, sort) and is the intended visual smoke target. Trigger to revisit: the next time `reacterm-demo.tsx` is edited for any reason, add a TreeTable section.
...
| 12 | Manual smoke via `examples/reacterm-demo.tsx` | ⚠ | Descoped — see Known Limitations entry 3. Playground example covers the same surface. |
