...
export function measureNaturalHeight(
  ...
  if (remaining >= 0) {
    ...
    if (totalFlexGrow > 0) {
      let distributed = 0;
      for (const entry of lineEntries) {
        if (entry.flexGrow <= 0) continue;
        const delta = Math.floor((entry.flexGrow / totalFlexGrow) * remaining);
        entry.allocatedWidth = clampSize(
          entry.allocatedWidth + delta,
          entry.child.props.minWidth,
          entry.child.props.maxWidth,
        );
        distributed += delta;
      }
      ...
      while (leftover > 0) {
        ...
        for (const entry of lineEntries) {
          ...
          if (max !== undefined && entry.allocatedWidth >= max) continue;
    ...
    if (totalShrinkWeight > 0) {
      for (const entry of lineEntries) {
        if (entry.flexShrink <= 0 || entry.allocatedWidth <= 0) continue;
        const shrink = Math.floor((entry.flexShrink * entry.allocatedWidth / totalShrinkWeight) * overflow);
        entry.allocatedWidth = clampSize(
          Math.max(0, entry.allocatedWidth - shrink),
          entry.child.props.minWidth,
          entry.child.props.maxWidth,
        );
      }
