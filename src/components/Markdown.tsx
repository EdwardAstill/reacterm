/**
 * Markdown — terminal-native Markdown renderer for Storm TUI.
 *
 * Parses a Markdown string and renders it as styled terminal UI using
 * Storm primitives (tui-box, tui-text). Supports headings, bold, italic,
 * strikethrough, code spans, fenced code blocks, links, images, unordered
 * and ordered lists (with nesting), task lists, blockquotes, horizontal
 * rules, tables, and paragraphs.
 *
 * The parser is self-contained — no external Markdown library is used.
 * It handles common Markdown patterns well but does not aim for full
...
export interface MarkdownProps extends StormLayoutStyleProps {
  ...
  content: string;
  /** Maximum width hint for wrapping (characters). */
  maxWidth?: number;
}
...
function parseBlocks(content: string): Block[] {
  ...
  function parseListItems(startIdx: number, baseIndent: number, ordered: boolean): { items: ListNode[]; endIdx: number } {
    ...
    while (idx < lines.length) {
      ...
      // Check for continuation / nested items
      while (idx < lines.length) {
...
export const Markdown = React.memo(function Markdown(rawProps: MarkdownProps): React.ReactElement {
  ...
  const props = usePluginProps("Markdown", rawProps as unknown as Record<string, unknown>) as unknown as MarkdownProps;
  const { content, maxWidth, color: _color, bold: _bold, dim: _dim, ...layoutProps } = props;
