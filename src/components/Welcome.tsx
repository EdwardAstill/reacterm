/**
 * Welcome — full-screen splash/welcome screen for Storm TUI apps.
 *
 * Renders a centered, full-screen welcome experience with optional
 * ASCII art logo, gradient app title, version/description, recent
 * items, keyboard shortcuts, and a dismiss prompt.
 *
 * Features:
 *   - Full-screen centered layout with optional background pattern
 *   - App title rendered bold with optional gradient colors
 *   - Version badge and description text
 *   - Recent items/actions section with selectable items
 *   - Keyboard shortcuts section (uses KeyboardHelp-style rendering)
 *   - Configurable dismiss prompt ("Press any key to continue" default)
 *   - onDismiss callback fires on any keypress or action selection
 *   - Optional ASCII art / logo block above the title
 *   - Plugin props and personality integration
 */
...
export const Welcome = React.memo(function Welcome(rawProps: WelcomeProps): React.ReactElement | null {
  ...
  const props = usePluginProps("Welcome", rawProps as unknown as Record<string, unknown>) as unknown as WelcomeProps;
  const { screen } = useTui();
    ...
    if (logoGradient && logoGradient.length >= 2) {
      sections.push(
        React.createElement(
          ...
          React.createElement(Gradient, { colors: logoGradient, direction: "vertical", children: logo } as any),
        ),
      );
    } else {
      sections.push(
  ...
  if (titleGradient && titleGradient.length >= 2) {
    sections.push(
      React.createElement(
        ...
        React.createElement(Gradient, { colors: titleGradient, children: title } as any),
      ),
    );
  } else {
    sections.push(
  ...
  if (shortcuts && shortcuts.length > 0) {
    ...
    for (let row = 0; row < perColumn; row++) {
      ...
      sections.push(
        React.createElement(
          "tui-box",
          { key: `srow-${row}`, flexDirection: "row", marginTop: row === 0 ? 0 : 0 },
          ...rowParts,
        ),
      );
    }
