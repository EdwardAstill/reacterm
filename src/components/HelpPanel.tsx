/**
 * HelpPanel — keyboard shortcut help overlay / inline panel.
 *
 * Displays categorized keyboard shortcuts with Kbd-styled key combos.
 * Can render as an inline panel or as a modal overlay.
 *
 * Features:
 *   - Category-grouped bindings with headers
 *   - Multi-column layout for dense shortcut lists
 *   - Optional search/filter to find shortcuts by key or description
 *   - Toggle visibility with a trigger key (default "?")
 *   - Scrollable when content exceeds available height
 *   - Inline or modal display mode
 */
...
export const HelpPanel = React.memo(function HelpPanel(rawProps: HelpPanelProps): React.ReactElement | null {
  ...
  const props = usePluginProps("HelpPanel", rawProps as unknown as Record<string, unknown>) as unknown as HelpPanelProps;
  ...
  if (description) {
    contentChildren.push(
      React.createElement(
        "tui-text",
        { key: "desc", color: colors.text.secondary, marginTop: title ? 0 : 0 },
        description,
      ),
    );
  }
