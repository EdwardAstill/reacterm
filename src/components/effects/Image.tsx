  ...
  try {
    ...
    if (bitDepth !== 8) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn(`[reacterm Image] Unsupported PNG bit depth: ${bitDepth} (only 8-bit is supported)`);
      }
      return null;
    }
    ...
    if (bpp === 0) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn(`[reacterm Image] Unsupported PNG color type: ${colorType} (only RGB/RGBA/grayscale supported)`);
      }
      return null;
    }
