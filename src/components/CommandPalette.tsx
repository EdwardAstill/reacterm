/**
 * CommandPalette — VS Code-style command palette overlay for Storm TUI.
 *
 * Opens as a top-center overlay triggered by a configurable key (default "/").
 * Features fuzzy search filtering, category-grouped command list with keyboard
 * navigation (Up/Down/Enter/Escape), optional icons and shortcut hints,
 * and a scrollable viewport with configurable maxVisible items.
 *
 * Architecture:
 *   - Uses useRef + useForceUpdate() for all mutable state (no useState).
 *   - Keyboard input via useInput with high priority to capture while open.
 *   - FocusGroup trap prevents focus leaking to components below the overlay.
 *   - Fuzzy matching scores on name, description, and category fields.
 *
 * Compound API:
 *   CommandPalette.Root — context provider with controlled open/close
 *   CommandPalette.Input — the search input row
 *   CommandPalette.List — the scrollable command list
 *   CommandPalette.Item — individual command row
 *
 * Recipe API (default export):
 *   <CommandPalette commands={[...]} onExecute={fn} />
 */
...
const CommandPaletteBase = React.memo(function CommandPalette(rawProps: CommandPaletteProps): React.ReactElement | null {
  const colors = useColors();
  const personality = usePersonality();
  const props = usePluginProps("CommandPalette", rawProps as unknown as Record<string, unknown>) as unknown as CommandPaletteProps;
  ...
  // ── Find next navigable index ──────────────────────────────
  const findNextNavigable = (items: PaletteCommand[], from: number, direction: 1 | -1): number => {
  ...
  if (filtered.length === 0) {
    ...
    // Track rendered category headers
