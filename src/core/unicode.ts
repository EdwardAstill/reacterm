...
function initBmpWidthTable(): void {
  ...
  const EMOJI_PRESENTATION_BMP: ReadonlyArray<readonly [number, number]> = [
    [0x231a, 0x231b], // ⌚ ⌛
    [0x23e9, 0x23ec], // ⏩ ⏪ ⏫ ⏬
    [0x23f0, 0x23f0], // ⏰
    [0x23f3, 0x23f3], // ⏳
  ...
  for (const [start, end] of EMOJI_PRESENTATION_BMP) {
    for (let i = start; i <= end; i++) BMP_WIDTH[i] = 2;
  }
}
...
function hasVS15(segment: string): boolean {
  for (let i = 0; i < segment.length; i++) {
    if (segment.charCodeAt(i) === VS15) return true;
  }
  return false;
}
