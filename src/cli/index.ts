...
program.command("run <entry>")
  ...
  .action(async (entry: string, opts: any) => {
    ...
    if (opts.capture) runOpts.capture = opts.capture;
    if (opts.includeSnapshots) runOpts.includeSnapshots = true;
    if (opts.redact) runOpts.redact = new RegExp(opts.redact);
...
function parseInlineSteps(argv: string[]): Step[] {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--press") { i++; const v = argv[i]!; steps.push({ kind: "press", keys: v.split(",") }); }
    else if (a === "--type") { i++; const v = argv[i]!; steps.push({ kind: "type", text: v }); }
    else if (a === "--paste") { i++; const v = argv[i]!; steps.push({ kind: "paste", text: v }); }
    else if (a === "--wait-for") { i++; const v = argv[i]!; steps.push({ kind: "waitFor", text: v }); }
    else if (a === "--sleep") { i++; const v = argv[i]!; steps.push({ kind: "sleep", ms: parseDuration(v) }); }
  }
  return steps;
}
