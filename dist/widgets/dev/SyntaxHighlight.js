...
function materializeTokens(line, refs) {
    const tokens = new Array(refs.length);
...
function startsWithAt(line, pos, prefix) {
    if (pos + prefix.length > line.length)
        return false;
    for (let i = 0; i < prefix.length; i++) {
        if (line.charCodeAt(pos + i) !== prefix.charCodeAt(i))
            return false;
    }
    return true;
}
...
function tokenizeLine(line, langDef, state) {
    ...
    const lcFirstCodes = new Array(langDef.lineComment.length);
    ...
    const stringCharCodes = new Array(langDef.stringChars.length);
    for (let i = 0; i < langDef.stringChars.length; i++) {
        stringCharCodes[i] = langDef.stringChars[i].charCodeAt(0);
    }
    const mlDelimFirstCodes = new Array(langDef.multilineStringDelims.length);
...
function classifyWord(word, firstCharCode, langDef) {
...
function matchOperator(line, pos) {
    const remaining = line.slice(pos);
...
function tokenizeTemplateLiteralContinuation(line, startPos, langDef, braceDepth) {
    const refs = [];
    let pos = startPos;
    const len = line.length;
    if (braceDepth > 0) {
...
function tokenizeTemplateLiteral(line, startPos, langDef) {
    const refs = [];
...
function tokenizeInterpolationRefs(line, startPos, langDef, initialDepth) {
    const refs = [];
    let pos = startPos;
...
function mapTreeSitterType(tsType) {
    switch (tsType) {
 ...
 */
function tokenizeWithTreeSitter(code, language) {
    const ts = getTreeSitter();
    if (!ts)
        return null;
    const lang = language.toLowerCase();
    if (!ts.isLanguageAvailable(lang))
        return null;
...
function renderToken(token, idx, colors) {
    const style = TOKEN_STYLES[token.kind];
    const props = { key: idx };
    if (style.color)
        props.color = style.color(colors.syntax);
    if (style.bold)
        props.bold = true;
    if (style.dim)
        props.dim = true;
...
function renderLine(lineTokens, lineIdx, colors) {
    const children = lineTokens.map((t, i) => renderToken(t, i, colors));
    return React.createElement(Text, { key: lineIdx }, ...children);
}
export function createSyntaxHighlightLines(code, language, colors) {
...
export const SyntaxHighlight = React.memo(function SyntaxHighlight(rawProps) {
    ...
    const lineElements = lines.map((lt, i) => lt.length > 0 ? renderLine(lt, i, colors) : React.createElement(Text, { key: i }, " "));
