 ...
 * ```
 */
export function screenshotPlugin(options = {}) {
    ...
    function bufferToSvg(buffer) {
        ...
        // Background
        parts.push(`<rect width="${svgWidth}" height="${svgHeight}" fill="${escapeXml(backgroundColor)}" />`);
    ...
    function buildTextSpan(col, y, text, fg, bold, charWidthPx) {
...
function fgHex(buffer, x, y) {
    const fg = buffer.getFg(x, y);
    if (fg === DEFAULT_COLOR)
        return "#D4D4D4"; // default terminal fg
    const r = (fg >> 16) & 0xff;
    const g = (fg >> 8) & 0xff;
    const b = fg & 0xff;
    return `#${r.toString(16).padStart(2, "0")}${g.toString(16).padStart(2, "0")}${b.toString(16).padStart(2, "0")}`;
}
function isBold(buffer, x, y) {
    // Attr.BOLD = 1 << 0 = 1
