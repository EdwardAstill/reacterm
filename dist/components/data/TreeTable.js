...
import { computeColumnWidths, fitColumnWidthsToAvailableWidth, buildSeparatorText, computeVirtualWindow, formatRowIndicator, headerTextWithSort, shouldStripe, } from "../../utils/table-render.js";
import { flattenVisible, MAX_TREE_DEPTH, } from "./TreeTable.flatten.js";
function buildTreePrefix(entry) {
...
export const TreeTable = React.memo(function TreeTable(rawProps) {
    const colors = useColors();
    const props = usePluginProps("TreeTable", rawProps);
    const { columns, data, treeColumnKey, onToggle, onRowSelect, onRowPress, onSort, onHeaderPress, isFocused = false, rowHighlight = false, sortable = false, stripe = false, headerColor = colors.brand.primary, maxVisibleRows = 100, scrollOffset = 0, onScrollChange, visibleWidth, stateStyles, renderCell, renderHeader, renderTreeCell, } = props;
    ...
    const resolvedTreeKey = (() => {
        ...
        if (!warnedTreeColumnRef.current) {
            ...
            console.warn(`[TreeTable] treeColumnKey "${treeColumnKey}" not found in columns; falling back to columns[0].`);
        }
        return columns[0]?.key;
    })();
    ...
    const baseColWidths = computeColumnWidths(columns, flat.map((f) => f.row.values), "header");
    ...
    const cycleSort = useCallback((columnKey) => {
    ...
    const mouseTarget = useMouseTarget({
        ...
        onMouse: (event, localX, localY) => {
            ...
            if (flatIndex < visibleStart || flatIndex >= visibleEnd || flatIndex >= totalFlat)
                return;
    ...
    function buildHeaderState(isCursorColOnHeader) {
        return mergeStyles(resolvedStateStyles.header, isCursorColOnHeader && isFocused ? resolvedStateStyles.focusedColumn : undefined);
    }
    const elements = [];
    if (renderHeader) {
        const headerCells = [];
        columns.forEach((col, ci) => {
            ...
            const inner = typeof rendered === "string"
                ...
                : rendered;
            headerCells.push(React.createElement("tui-box", { key: col.key, flexDirection: "row", width: (colWidths[ci] ?? 0) + 2 }, inner));
    ...
    elements.push(React.createElement("tui-text", { key: "sep", color: borderColor }, separatorText));
    for (let i = visibleStart; i < visibleEnd; i++) {
        ...
        const rowBoxProps = { key: `row-${i}`, flexDirection: "row" };
        ...
        const cellElements = columns.flatMap((col, ci) => {
            ...
            const style = mergeStyles(col.color !== undefined || col.bold !== undefined || col.dim !== undefined
                ? {
                ...
                : undefined, state.isFocusedRow ? resolvedStateStyles.focusedRow : undefined, state.isFocusedCell ? resolvedStateStyles.focusedCell : undefined);
            ...
            if (ci < columns.length - 1) {
                nodes.push(React.createElement("tui-text", { key: `sep-${i}:${col.key}`, color: borderColor }, "│"));
            }
            return nodes;
        });
    ...
    if (needsVirtualization) {
        elements.push(React.createElement("tui-text", { key: "row-indicator", color: colors.text.dim, dim: true }, formatRowIndicator(visibleStart, visibleEnd, totalFlat, "Showing ")));
    }
