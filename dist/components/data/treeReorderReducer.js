export const initialReorderState = { phase: "idle" };
export function reorderReducer(state, action, nodes, canMove, priorScratch, priorEphemeral) {
...
function handleLiveMotion(state, action, nodes, canMove, priorScratch, priorEphemeral) {
    const baseline = priorScratch ?? nodes;
    const build = (scratch, ephemeral) => ephemeral !== undefined
    ...
    if (!path)
        return build(baseline, priorEphemeral);
    const noop = () => build(baseline, priorEphemeral);
    ...
    if (action.type === "outdent") {
        ...
        const grandparentKey = grandparentPath.length === 0 ? null : getNodeAtPath(baseline, grandparentPath)?.key ?? null;
        if (canMove &&
...
function commitLive(state, _cursorKey, nodes, priorScratch, priorEphemeral) {
    ...
    const targetParentKey = finalParentPath.length === 0 ? null : getNodeAtPath(nextNodes, finalParentPath)?.key ?? null;
    const targetIndex = finalPath[finalPath.length - 1];
    const origParentPath = origPath.slice(0, -1);
    const origParentKey = origParentPath.length === 0 ? null : getNodeAtPath(nodes, origParentPath)?.key ?? null;
...
function commitStash(state, cursorKey, nodes) {
...
function removeAtPath(tree, path) {
    ...
    if (!removed)
        return { next: tree, removed: null };
    const nextNode = { ...child, children: nextChildren };
    const next = tree.slice();
    next[idx] = nextNode;
    return { next, removed };
}
function insertAtPath(tree, path, node) {
    if (path.length === 0)
        return tree;
    const [head, ...rest] = path;
    const idx = head;
...
function swapSiblings(tree, parentPath, i, j) {
...
function topmostOnly(keys, nodes) {
...
//# sourceMappingURL=treeReorderReducer.js.map