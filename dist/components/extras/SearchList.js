...
import { computeScrollWindow, findFirstNavigable, findNextNavigable, } from "../../utils/navigation.js";
 ...
 * />
 */
export function SearchList(rawProps) {
    const colors = useColors();
    const personality = usePersonality();
    const props = usePluginProps("SearchList", rawProps);
    const { items, onSelect, onChange, onCancel, isFocused = true, placeholder = "Search…", maxVisible = DEFAULT_MAX_VISIBLE, indicator, activeColor = colors.brand.primary, iconColor = colors.text.dim, searchIcon = DEFAULT_SEARCH_ICON, showResultCount = true, filter, hideListUntilQuery = false, debounceMs = 0, } = props;
    ...
    const activeIndexRef = useRef(findFirstNavigable(items.length, (i) => isNavigable(items[i])));
    ...
    if (lastItemsRef.current !== items) {
        ...
        if (activeIndexRef.current >= filteredRef.current.length) {
            activeIndexRef.current = findFirstNavigable(filteredRef.current.length, (i) => isNavigable(filteredRef.current[i]));
        }
    }
    const recomputeFiltered = useCallback(() => {
        filteredRef.current = applyFilter(itemsRef.current, queryRef.current, filterRef.current);
        ...
        if (filteredRef.current.length === 0) {
            ...
            activeIndexRef.current = findNextNavigable(filteredRef.current.length, activeIndexRef.current, 1, (i) => isNavigable(filteredRef.current[i]));
        }
    }, []);
    ...
    const handleInput = useCallback((event) => {
        if (event.key === "up") {
            ...
            if (list.length === 0)
                return;
            const next = findNextNavigable(list.length, activeIndexRef.current, -1, (i) => isNavigable(list[i]));
            if (next !== activeIndexRef.current) {
                activeIndexRef.current = next;
                const item = list[next];
                if (item)
                    onChangeRef.current?.(item.value, item);
                forceUpdate();
                requestRender();
            }
            return;
    ...
    if (showResultCount && query.length > 0) {
        inputRow.push(React.createElement("tui-text", { key: "count", color: colors.text.dim, dim: true }, ` ${filteredItems.length} result${filteredItems.length === 1 ? "" : "s"}`));
    }
    const showList = !(hideListUntilQuery && query.length === 0);
    const listElements = [];
    if (showList) {
        if (filteredItems.length === 0) {
            listElements.push(React.createElement("tui-text", { key: "empty", color: colors.text.dim, dim: true }, query.length > 0 ? "  No matches" : "  No options"));
        }
        else {
            const { start, end } = computeScrollWindow(filteredItems.length, activeIndexRef.current, maxVisible);
            const visible = filteredItems.slice(start, end);
            const overflowTop = start > 0;
            const overflowBottom = end < filteredItems.length;
            if (overflowTop) {
                listElements.push(React.createElement("tui-text", { key: "__top-ovf", color: colors.text.dim, dim: true }, "  ···"));
            }
            visible.forEach((item, i) => {
                ...
                if (item.richLabel && !disabled) {
                    listElements.push(React.createElement("tui-box", { key: item.value, flexDirection: "row" }, React.createElement("tui-text", { key: "ind", color: isActive ? activeColor : textColor, bold: isActive }, indicatorStr), React.createElement(React.Fragment, { key: "rich" }, item.richLabel)));
                }
                else {
                    listElements.push(React.createElement("tui-text", {
                        key: item.value,
                        color: textColor,
                        bold: isActive,
                        dim: disabled,
            ...
            if (overflowBottom) {
                listElements.push(React.createElement("tui-text", { key: "__bot-ovf", color: colors.text.dim, dim: true }, "  ···"));
            }
        }
    }
    ...
    }, React.createElement("tui-box", { key: "input", flexDirection: "row", role: "search" }, ...inputRow), ...listElements);
}
