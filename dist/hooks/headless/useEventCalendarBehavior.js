...
function buildVisibleDates(start, end) {
    const dates = [];
...
function getVisibleRange(anchorDate, view, weekStartsOn, agendaDays) {
    if (view === "week") {
        return {
...
function getTitle(anchorDate, view, weekStartsOn, agendaDays) {
    if (view === "month") {
...
function overlapsDay(event, date) {
    const dayStart = startOfDay(date).getTime();
    const dayEnd = endOfDay(date).getTime();
    return event.start.getTime() <= dayEnd && event.end.getTime() >= dayStart;
}
export function useEventCalendarBehavior(options = {}) {
    const { events = [], anchorDate: controlledAnchorDate, defaultAnchorDate, onAnchorDateChange, view: controlledView, defaultView = "month", onViewChange, weekStartsOn = 0, agendaDays = 14, today: todayProp, } = options;
    const [uncontrolledAnchorDate, setUncontrolledAnchorDate] = useState(startOfDay(defaultAnchorDate ?? controlledAnchorDate ?? todayProp ?? new Date()));
    const [uncontrolledView, setUncontrolledView] = useState(controlledView ?? defaultView);
    ...
    const getEventsForRange = (start, end) => {
        ...
        return normalizedEvents.filter((event) => event.start.getTime() <= endTime && event.end.getTime() >= startTime);
    };
    const getEventsForDay = (date) => normalizedEvents.filter((event) => overlapsDay(event, date));
    return {
        anchorDate,
        view,
        weekStartsOn,
...
//# sourceMappingURL=useEventCalendarBehavior.js.map