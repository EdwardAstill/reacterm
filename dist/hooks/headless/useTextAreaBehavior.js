...
export function useTextAreaBehavior(options) {
    const { value, onChange, onSubmit, submitKey = "ctrl+enter", isFocused: isFocusedProp = true, readOnly = false, disabled = false, tabSize = 2, maxLines, maxLength, onSelectionChange, } = options;
    const { input, focus, requestRender } = useTui();
    // ── Refs for mutable state ─────────────────────────────────────
    const linesRef = useRef(splitLines(value));
    ...
    if (!keyRegistered.current) {
        ...
        // ── Scroll adjustment ──────────────────────────────────────
        const ensureCursorVisible = (cursorRow) => {
        ...
        const isSubmitKey = (event) => {
            ...
            if (sk === "ctrl+enter" && event.ctrl && event.key === "return")
                return true;
            if (sk === "meta+enter" && event.meta && event.key === "return")
                return true;
            if (sk === "ctrl+s" && event.ctrl && event.key === "s")
                return true;
            return false;
        };
        // ── Main key handler ───────────────────────────────────────
        unsubKeyRef.current = input.onKey((event) => {
            ...
            if (event.shift && (event.key === "left" || event.key === "right" ||
                ...
                if (event.key === "left") {
                    ...
                    if (cur.row > 0) {
                        if (stickyColRef.current === null)
                            stickyColRef.current = cur.col;
                        cur.row--;
            ...
            if (event.key === "return") {
                ...
                if (ml !== undefined && joinLines(lines).length >= ml)
                    return;
            ...
            if (event.key === "tab") {
                ...
                if (ml !== undefined && currentLen + spaces.length > ml)
                    return;
                const line = lines[cur.row];
                lines = [...lines];
...
//# sourceMappingURL=useTextAreaBehavior.js.map