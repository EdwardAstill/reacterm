...
function wrapText(text, rowWidth) {
    if (rowWidth <= 0 || text.length === 0)
        return [""];
    const rows = [];
...
function buildRowOffsets(text, rows) {
    const offsets = [];
...
function cursorToRowCol(cursor, rows, text) {
...
function rowColToCursor(row, col, rows, text) {
    if (text !== undefined && text.includes("\n")) {
        const offsets = buildRowOffsets(text, rows);
...
export function useChatInputBehavior(options) {
    const { value, onChange, onSubmit, isFocused: focusProp = true, history = [], maxRows = 4, maxLength, multiline = false, disabled = false, onSelectionChange, width: widthOption, } = options;
        ...
        unsubKeyRef.current = input.onKey((event) => {
            ...
            if (event.ctrl || event.meta) {
                ...
                if (event.key === "left") {
                    ...
                    while (newCursor > 0 && val[newCursor - 1] === " ")
                        newCursor--;
                    while (newCursor > 0 && val[newCursor - 1] !== " ")
                        newCursor--;
...
//# sourceMappingURL=useChatInputBehavior.js.map