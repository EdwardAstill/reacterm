...
import { createStyleSheet } from "../core/stylesheet.js";
import { createStyleSheetLoader, } from "../core/stylesheet-loader.js";
import { extractThemeOverrides } from "../theme/index.js";
/**
 * Convert parsed rules (selector + Record<string, unknown>) into the
 * typed StyleRule records that StyleSheet.create() expects.
 */
function toStyleSheetRules(parsed) {
    const rules = {};
 ...
 */
export function useStyleSheet(options) {
    ...
    const { stylesheet, close } = createStyleSheetLoader({
        ...
        onReload: (newParsed) => {
            ...
            StyleContext._currentValue = newSheet; // React private API — imperative context update
            StyleContext._currentValue2 = newSheet; // React private API — imperative context update
    ...
    StyleContext._currentValue = sheet; // React private API — imperative context update
    StyleContext._currentValue2 = sheet; // React private API — imperative context update
