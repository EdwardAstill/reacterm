...
export async function parseScenario(source, filename) {
    let raw;
    ...
    if (!parsed.success) {
        const messages = parsed.error.issues.map(i => {
            ...
            if (pathStr.startsWith("steps.") && i.message === "Invalid input" && Array.isArray(raw?.steps)) {
                const idx = parseInt(i.path[1], 10);
                const stepObj = raw.steps[idx];
