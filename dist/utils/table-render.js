 ...
 */
export function computeColumnWidths(columns, data, headerField = "header", extraPadding = 0) {
    const sampleCount = Math.min(data.length, COL_WIDTH_SAMPLE_SIZE);
    return columns.map((col) => {
        if (col.width !== undefined)
            return col.width;
        const headerText = String(col[headerField] ?? "");
 ...
 */
export function fitColumnWidthsToAvailableWidth(columns, widths, availableWidth, minColumnWidth = 1) {
    ...
    const maxContentWidth = Math.max(minColumnWidth * columns.length, availableWidth - separatorWidth - paddingWidth);
    const next = [...widths];
 ...
 */
export function buildSeparatorText(colWidths, joiner) {
    return colWidths
        .map((w) => "\u2500".repeat(w + 2))
        .join(joiner);
}
 ...
 */
export function shouldStripe(rowIndex, stripeEnabled, isHighlighted) {
    return stripeEnabled && rowIndex % 2 === 1 && !isHighlighted;
}
/**
 * Compute the visible row window for virtualized rendering.
 */
export function computeVirtualWindow(totalRows, maxVisibleRows, scrollOffset) {
 ...
 */
export function formatRowIndicator(start, end, total, prefix = "") {
 ...
 */
export function headerTextWithSort(label, columnKey, sortColumn, sortDirection) {
 ...
 */
export function buildRowLine(columns, row, colWidths, separator = "\u2502") {
...
//# sourceMappingURL=table-render.js.map