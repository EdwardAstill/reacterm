...
function parseSGR(buffer) {
    ...
    const action = release ? "release" : baseAction;
    return {
        event: {
            button,
            action,
...
function parseX11(buffer) {
    if (!buffer.startsWith(X11_PREFIX))
        return null;
    if (buffer.length < X11_PREFIX.length + 3)
        return null;
...
function decodeButton(cb) {
    ...
    if (isScroll) {
        // Scroll wheel
        if (low2 === 0)
            return { button: "scroll-up", action: "press" };
        if (low2 === 1)
            return { button: "scroll-down", action: "press" };
        if (low2 === 2)
            return { button: "scroll-left", action: "press" };
        return { button: "scroll-right", action: "press" };
    }
    if (isMotion) {
        // Motion event
        const motionButton = low2 === 0 ? "left" : low2 === 1 ? "middle" : low2 === 2 ? "right" : "none";
        return { button: motionButton, action: "move" };
    }
    // Button press
    if (low2 === 0)
        return { button: "left", action: "press" };
    if (low2 === 1)
        return { button: "middle", action: "press" };
    if (low2 === 2)
        return { button: "right", action: "press" };
 ...
 */
export function looksLikeMalformedSgrMouse(buffer) {
    if (!buffer.startsWith("\x1b[<"))
        return false;
